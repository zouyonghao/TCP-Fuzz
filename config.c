...
struct option options[] = {
	...
	{ "fuzz_file_name",	.has_arg = true,  NULL, OPT_FUZZ_FILE_NAME },
	{ "tun_id",	.has_arg = true,  NULL, OPT_TUN_ID },
			   ...
			   char *where)
{
	...
	case OPT_VERBOSE:
		config->verbose = true;
		break;
	case OPT_FUZZ_FILE_NAME:
		fuzz_file_name = strdup(optarg);
		break;
	case OPT_TUN_ID:
		config->tun_id = atoi(optarg);
		break;
	case OPT_NO_FUZZ_SYSCALL:
		config->is_no_fuzz_syscall = true;
		break;
	case OPT_NO_FUZZ_PACKET:
		config->is_no_fuzz_packet = true;
		break;
