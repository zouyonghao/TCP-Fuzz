...
{
// #ifdef DISABLE_CHECK_EVENT_TIME
// 	// disable check_event_time
// 	return STATUS_OK;
// #endif
...
void wait_for_event(struct state *state)
{
	...
	// printf("waiting until %lld -- now is %lld\n",
	//        event_usecs, now_usecs(state));
	while (1) {
...
void run_local_packet_event(struct state *state, struct event *event,
 ...
 */
s64 schedule_start_time_usecs(struct state *state)
{
#ifdef linux
	s64 start_usecs = 0;
