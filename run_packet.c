 ...
 */
struct socket *find_socket_for_live_packet(
...
int tcp_convert_seq_number(struct socket *socket, struct packet *packet,
				  char **error)
{
	...
	struct packet *script_packet, char **error)
{
	...
	// }
		...
		u32 seq = ntohl(actual_packet->tcp->seq);
		u8 *live_payload = packet_payload(actual_packet);
		for (int i = 0; i < packet_payload_len(actual_packet); i++) {
			if (live_payload[i] != (seq - 1 + i) % 10) {
				fprintf(stderr, "outbound packet data fault!\n");
				fflush(stderr);
				abort();
			}
		}
...
int find_or_create_socket_for_script_packet(
	struct state *state, struct packet *packet,
	...
	struct socket *socket,	char **error)
{
	...
	// printf("do_inbound_script_packet\n");
	int result = STATUS_ERR;	/* return value */
