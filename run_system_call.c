...
{
	add_syscall_result_to_fuzz_result(syscall->name, actual);
			 ...
			 struct expression_list *args, char **error)
{
	...
	if (result > 0)
		total_write_num += result;
	DEBUG_FUZZP("total_write_num: %llu\n", total_write_num);
			 ...
			 struct expression_list *args, char **error)
{
	...
	if (actual_arg_count == 2) {
		...
		add_syscall_result_to_fuzz_result("ioctl_param", live_optval);
